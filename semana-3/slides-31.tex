...
\begin{frame}[allowframebreaks,fragile]{funções}
  ...
  Funções podem não receber argumentos. Nesse caso, usamos
  \ilcode{void} nos parâmetros para declarar explicitamente que a
  função não aceita nenhuma entrada:
  ...
  Em C, \ilcode{int f()} (parênteses vazios) \textbf{não} significa
  ``zero argumentos'' --- significa ``número indeterminado de
  argumentos''. Usar \ilcode{void} é a forma correta e precisa.

  \framebreak

  Quando usar \ilcode{void} nos parâmetros? Quando a função obtém seus
  dados por outros meios (lendo do teclado, acessando variáveis
  globais etc) ou simplesmente não precisa de entrada.
  ...
  Da mesma forma, funções podem não retornar valor. Declaramos que o
  retorno é \ilcode{void}:
...
\begin{frame}[allowframebreaks,fragile]{níveis de abstração}
  ...
  \lstinputlisting[style=normalc,caption=meow2.c]{meow3.c}
  ...
  \lstinputlisting[style=normalc,caption=meow3.c]{meow4.c}
...
\begin{frame}[allowframebreaks,fragile]{parâmetros e argumentos}
  ...
  \begin{lstlisting}[style=normalc,caption=mario.c]
    int get_size(void);
    void print_grid(int n);
