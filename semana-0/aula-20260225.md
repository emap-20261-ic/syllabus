...
Podemos redirecionar a saída de comandos para arquivos, em vez de
exibi-la na tela:
...
$ wc -l < nomes.txt # conteúdo passado na entrada padrão
$ wc -l nomes.txt   # como argumento
```
...
é feito com o operador `|` (pipe). No exemplo `sort nomes.txt | uniq`,
a saída de `sort` (que iria para a tela) é enviada para a entrada do
`uniq`.

Podemos também encadear comandos com operadores lógicos:
...
Vejamos mais alguns exemplos práticos. Suponha que temos um arquivo
...
### Opções e parâmetros de comandos

Nos exemplos acima, usamos construções como `wc -l`, `sort -rn` e
`uniq -c`. As letras precedidas por `-` são **opções** (ou *flags*)
que modificam o comportamento dos comandos. Esta convenção vem dos
primeiros sistemas Unix, nos anos 1970: o hífen foi escolhido por ser
fácil de digitar e raramente aparecer no início de nomes de arquivos.

**Opções curtas** usam um único hífen e uma letra:
...
Opções curtas podem ser **agrupadas** numa única sequência:

``` bash
$ ls -la         # equivale a ls -l -a
$ tar -xzvf arquivo.tar.gz   # equivale a tar -x -z -v -f arquivo.tar.gz
```

Nos anos 1980, o projeto GNU introduziu **opções longas** com dois
hífens (`--`), que tornam os comandos mais legíveis:

``` bash
$ ls --all                   # equivale a ls -a
$ grep --ignore-case "foo" arquivo.txt   # equivale a grep -i "foo" arquivo.txt
```

O `--` sozinho tem outro papel: indica o **fim das opções**. Isto é
útil quando um argumento começa com hífen e poderia ser confundido com
uma opção:

``` bash
$ rm -- -arquivo    # remove o arquivo chamado "-arquivo"
```

Resumindo as convenções:

| Forma   | Significado                    |
|---------|--------------------------------|
| `-a`    | opção curta                    |
| `-abc`  | várias opções curtas agrupadas |
| `--all` | opção longa                    |
| `--`    | fim das opções                 |

Uma curiosidade: no DOS/Windows antigo, opções usavam `/` em vez de
`-` (por exemplo, `dir /p`), justamente porque no Unix a `/` já era
usada como separador de diretórios.
...
Estes são programas mais complexos que não vamos explorar em
...
5. **Trazer alterações do repositório remoto**:

``` bash
$ git pull
```

Este comando busca os commits que existem no repositório remoto mas
ainda não estão na sua cópia local, e os integra ao seu histórico. É o
complemento do `git push`: enquanto o push envia suas alterações para o
remoto, o pull traz as alterações do remoto para você. Use-o sempre que
quiser atualizar sua cópia local com o que foi publicado por outras
pessoas (ou por você mesmo a partir de outra máquina).
