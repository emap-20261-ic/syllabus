...
\begin{frame}[allowframebreaks]{o que é programação}
  ...
  O método de Newton nos dá um \textbf{procedimento} para encontrar
  $\sqrt{x}$: começando com um palpite $y$, melhoramos a aproximação
  fazendo a média entre $y$ e $x/y$, repetindo até convergir. Qual a
  raiz de 2?
  ...
  \lstinputlisting[style=normalc,caption=src/root.c]{src/root.c}

  Diferente da definição matemática, o programa descreve \textbf{como}
  encontrar a raiz quadrada --- um \textbf{algoritmo}.
...
\begin{frame}[allowframebreaks]{Programas}
  ...
  \href{https://en.wikipedia.org/wiki/C_(programming_language)}{C} é
  uma das linguagens mais
  ...
  Laboratories. Vide
  \href{https://youtu.be/G1-wse8nsxY?si=lx0ygn0QGbFg_Bz0}{C
    Programming Language, Brian Kernighan and Lex Fridman}.
  ...
  Os padrões da linguagem C evoluíram: C89/C90 (primeiro padrão
  ANSI/ISO), C99, C11, C17 e C23.

  Neste curso usaremos C17.
