...
\begin{frame}[fragile,allowframebreaks]{Selection Sort}
  ...
  \begin{lstlisting}[style=normalc,caption=selection.c]
...
\begin{frame}{Exemplos extras}
  ...
  \begin{itemize}
    \item \texttt{recursao0.c} — fatorial: forma canônica de recursão
    \item \texttt{recursao1.c} — soma de 1 a n: mesmo padrão, operação diferente
    \item \texttt{recursao2.c} — fatorial com acumulador (tail recursion)
    \item \texttt{steps.c}
    \item \texttt{numeros.txt} (entrada de exemplo)
  \end{itemize}
\end{frame}
